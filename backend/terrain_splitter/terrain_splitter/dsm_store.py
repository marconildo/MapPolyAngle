    ...
    with tifffile.TiffFile(io.BytesIO(payload)) as tf:
        ...
        if extrasamples:
            ...
            for extra_index, extra_sample in enumerate(extrasamples):
                ...
                if "alpha" not in extra_name and "mask" not in extra_name:
                    continue
...
class DsmDatasetStore:
    ...
    def ingest_dataset(
        ...
        with self._lock:
            ...
            normalized, valid_mask = _read_raster_and_valid_mask(payload, source_descriptor)
            ...
            descriptor = self._build_descriptor(dataset_id, original_name, len(payload), source_descriptor, valid_mask)
    ...
    def get_dataset_descriptor(self, dataset_id: str) -> DsmSourceDescriptorModel | None:
        with self._lock:
            ...
            if not isinstance(entry, dict):
                return None
            return DsmSourceDescriptorModel.model_validate(entry["descriptor"])
...
class S3BackedDsmDatasetStore(DsmDatasetStore):
    ...
    def ingest_dataset(
        ...
        suffix = Path(original_name or source_descriptor.name or "surface.tiff").suffix or ".tiff"
        with self._lock:
            existing_descriptor = self._load_remote_descriptor(dataset_id)
            if existing_descriptor is not None:
                return existing_descriptor, True

            stored_descriptor, reused_existing = super().ingest_dataset(payload, original_name, source_descriptor)
            if reused_existing:
            ...
            client.put_object(
                Bucket=self.bucket,
                Key=self._source_key(dataset_id, suffix),
                Body=file_path.read_bytes(),
                ContentType="image/tiff",
            )
            client.put_object(
                Bucket=self.bucket,
                Key=self._pyramid_key(dataset_id),
                Body=pyramid_path.read_bytes(),
                ContentType="application/octet-stream",
            )
            client.put_object(
                Bucket=self.bucket,
                Key=self._descriptor_key(dataset_id),
                Body=stored_descriptor.model_dump_json().encode("utf-8"),
                ContentType="application/json",
            )
            self._upsert_local_index_entry(dataset_id, stored_descriptor, suffix)
    ...
    def get_dataset_descriptor(self, dataset_id: str) -> DsmSourceDescriptorModel | None:
        descriptor = super().get_dataset_descriptor(dataset_id)
        if descriptor is not None:
            return descriptor
        return self._load_remote_descriptor(dataset_id)
